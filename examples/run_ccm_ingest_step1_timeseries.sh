#!/usr/bin/env bash
# ==============================================================================
# run_ccm_ingest_step1_timeseries.sh — CCM 结果导入 Step 1：生成标准 timeseries。
#
# 读取 runs/<case>/out_put 下的 STAR-CCM+ monitor CSV，并结合该 case 的
# input/actuation_schedule.csv，把整个目录原地整理成标准 case 结构：
#   timeseries.csv
#   actuation_schedule.csv
#   input/actuation_schedule.csv
#   case_manifest.yaml
#   quality_report.json
#   figures/ logs/ flow_snapshots/
#
# 同时生成一组与 mock 输出同名的快速 SVG 数据图；Step 2 再做质量检查，
# Step 3 再生成质量检查后的 PNG 诊断图。
# ==============================================================================
set -euo pipefail
...
while IFS= read -r case_dir; do
  ...
  find runs -mindepth 2 -maxdepth 2 -type d -name out_put \
    -print \
    | sed 's#/out_put$##' \
    | sort
)
...
if [[ "${#CASE_DIRS[@]}" -eq 0 ]]; then
  echo "未找到 CCM 输出目录。需要 runs/<目录>/out_put。"
  exit 1
fi

echo "当前可生成标准 timeseries 的 CCM case 目录路径："
printf '%s\n' "${CASE_DIRS[@]}"
...
if [[ ! -d "${case_dir}/out_put" ]]; then
  echo "未找到 CCM 输出目录：${case_dir}/out_put"
  exit 1
fi
...
from flow_control.mock.mock_plant import (
...
window_duration = (
    float_value(schedule_rows[0], "t_end") - float_value(schedule_rows[0], "t_start")
)
jet_amplitude = max(
    sum(float_value(row, col) for col in MASSFLOW_COLUMNS)
    for row in schedule_rows
)
